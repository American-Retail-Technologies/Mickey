...
class Data extends AbstractHelper
{
	...
	public function _getCfg($attr = null , $data = null)
	{
		...
		$defaults = [];
		$collection = $this->_scopeConfigInterface->getValue('cartpro');
		...
		$_configs = $this->_scopeConfigInterface->getValue('cartpro');
		if (empty($_configs)) return;
		$cfgs = [];

		foreach ($groups as $group) {
			$_cfgs = $this->_scopeConfigInterface->getValue('cartpro/'.$group.'');
	...
	private function getProductImages($product, $_config, $prefix = 'img')
	{
		...
		$_placeholder = ($placeholder != '' && strpos($placeholder, 'http') !== false) ? $placeholder :  $this->_block->getViewFileUrl('Sm_Cartpro::images/nophoto.jpg');
