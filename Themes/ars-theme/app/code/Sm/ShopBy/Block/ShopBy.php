...
use Sm\ShopBy\Block\Cache\Lite;

class ShopBy extends \Magento\Catalog\Block\Product\AbstractProduct
{
	...
	protected function _toHtml()
	{
		...
		$folder_cache = $folder_cache.'Sm/ShopBy/';
		...
		$Cache_Lite = new \Sm\ShopBy\Block\Cache\Lite($options);
		if ($use_cache){
			...
			if ($data = $Cache_Lite->get($hash)) {
				...
				$template_file = (!empty($template_file)) ? $template_file : "Sm_ShopBy::default.phtml";
				...
				if ($products != null) {
					...
					if (!empty($_products)) {
						foreach ($_products as $_product) {
							...
							$_product->created_at = $_product->getCreatedAt();
							...
							$_product->cat_id = $category->getId();
						}
						return $_products;
					}
				}
	...
	{
		$todayStartOfDayDate = $this->_localeResolver->getLocale()->date()->setTime(0, 0, 0)->format('Y-m-d H:i:s');
        $todayEndOfDayDate = $this->_localeResolver->getLocale()->date()->setTime(23, 59, 59)->format('Y-m-d H:i:s');
	...
	private function _addOrderedCount(& $collection)
	{
		...
		$select = $this->_resource->getConnection('core_read')
			->select()
			->from($order_item_table, ['product_id', 'ordered_qty' => 'SUM(`qty_ordered`)'])
			->group('product_id');
	...
	public function _getProductsIDs()
	{
		...
		$products = $this->_objectManager->get('Magento\Catalog\Model\Product')
				->getCollection()
				->addAttributeToSelect('*')->addIdFilter($catids);	
